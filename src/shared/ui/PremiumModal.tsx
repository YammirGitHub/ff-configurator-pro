  ...
  const handleRedeem = async () => {
    ...
    try {
      ...
      if (auth.currentUser) {
        ...
        // 👇 SENIOR UX: Pantalla de éxito en lugar de alert()
        setSuccessMode(true);
        ...
        >
          ...
          >
            <GlassCard className="relative flex flex-col h-full border-[#ffd700]/20 shadow-[0_0_60px_rgba(255,107,53,0.1)] p-0">
              {/* PANTALLA DE ÉXITO (Reemplaza al alert "Bienvenido") */}
              {successMode ? (
                <div className="flex flex-col items-center justify-center p-12 text-center h-[400px]">
                  <motion.div
                    initial={{ scale: 0 }}
                    animate={{ scale: 1 }}
                    className="flex h-20 w-20 items-center justify-center rounded-full bg-[#25D366]/20 text-[#25D366] text-4xl mb-6"
                  >
                    ✓
                  </motion.div>
                  <h2 className="font-display text-2xl font-black text-white">
                    ¡ACTIVACIÓN VIP EXITOSA!
                  </h2>
                  <p className="text-zinc-400 mt-2 text-sm">
                    Recargando el sistema maestro...
                  </p>
                </div>
              ) : (
                <>
                    ...
                    <div className="bg-[#141728]/80 rounded-2xl border border-white/5 p-4 sm:p-6 mb-6">
                      <div className="flex flex-col items-center gap-4 text-center">
                        ...
                        <div className="space-y-2">
                          ...
                          <p className="font-mono text-sm font-bold text-[#ff6b35] flex items-center justify-center gap-2 mt-1">
                            {yapeNumber}
                            {/* 👇 REEMPLAZA EL alert("Copiado") */}
                            <button
                              onClick={handleCopy}
                              className={`px-2 py-1 rounded text-[10px] font-black uppercase transition-all ${copied ? "bg-[#25D366]/20 text-[#25D366]" : "bg-white/5 text-white hover:bg-white/10"}`}
                            >
